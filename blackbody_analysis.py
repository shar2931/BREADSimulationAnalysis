...
numFiles = 11
fpath = 'BlackbodyStudiesData/2024-07-09/'
fname = 'SNSPD-'
...
if normalized:
    errS = np.sqrt(np.square(np.divide(errS, fS[8])), np.square(np.divide(np.multiply(errS[6], errS), fS[6]**2)))
    fS = np.multiply(fS, 1 / fS[8])
...
if normalized:
    errM = np.sqrt(np.square(np.divide(errM, fM[8])), np.square(np.divide(np.multiply(errM[6], errM), fM[6]**2)))
    fM = np.multiply(fM, 1 / fM[8])
...
if normalized:
    errL = np.sqrt(np.square(np.divide(errL, fL[8])), np.square(np.divide(np.multiply(errL[6], errL), fL[6]**2)))
    fL = np.multiply(fL, 1 / fL[8])
...
if normalized:
    ...
    plt.xticks(np.arange(-12, 10, step=4))
    plt.legend()
    plt.savefig('BlackbodyPlots/2024-07-09/z-axis-irradiance-normalized.png')
    ...
    plt.legend()
    plt.savefig('BlackbodyPlots/2024-07-09/z-axis-irradiance.png')
